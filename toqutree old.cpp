...
/* your code here */
	...
	imgCopy = PNG(imIn);
	//cout << "resize copy" << endl;
	//cout << "PASSED HERE" << endl;
	PNG* treeImg = new PNG(pow(2,k), pow(2,k));

	if(width != height || (width == height && width != pow(2,k))) {
		int length = pow(2,k);
		for(unsigned int i = (imIn.width() - length) / 2; i < (imIn.width() - length) / 2 + length; i++) {
			for(unsigned int j = (imIn.height() - length) / 2; j < (imIn.height() - length) / 2 + length; j++) {
				HSLAPixel* p = treeImg->getPixel(i - (imIn.width() - length) / 2, j - (imIn.height() - length) / 2);
				*p = *imIn.getPixel(i,j);
                p = NULL;               //free
			}
		}
...
// an average.
    ...
    for(int i = pow(2,k-2); i < 3*pow(2,k-2); i++){
        for(int j = pow(2,k-2); j < 3*pow(2,k-2); j++){
			...
			double entropyNW = s.entropy(SW_ul, SW_lr);
	...
	PNG* imgSE = new PNG(im->width()/2, im->height()/2);
    for(unsigned int i = 0; i < im->width()/2; i++){
        for(unsigned int j = 0; j < im->height()/2; j++){
	...
	int SW_x = (SE_x + a) % d;
	int SW_y = SE_y;
	PNG* imgSW = new PNG(im->width()/2, im->height()/2);
    for(unsigned int i = 0; i < im->width()/2; i++){
        for(unsigned int j = 0; j < im->height()/2; j++){
	...
	int NE_x = SE_x;
	int NE_y = (SE_y + a) % d;
	PNG* imgNE = new PNG(im->width()/2, im->height()/2);
    for(unsigned int i = 0; i < im->width()/2; i++){
        for(unsigned int j = 0; j < im->height()/2; j++){
	...
	int NW_x = (SE_x + a) % d;
	int NW_y = (SE_y + a) % d;
	PNG* imgNW = new PNG(im->width()/2, im->height()/2);
    for(unsigned int i = 0; i < im->width()/2; i++){
        for(unsigned int j = 0; j < im->height()/2; j++){
    ...
    //cout << ret->SE->avg.dist(ret->avg) << endl;
	delete im;
	im = NULL;
...
/* your code here */
    ...
    PNG * ret = new PNG(imgCopy);
	cout << "HERE" << endl;
	int length = pow(2, root->dimension);
    for(unsigned int x = (imgCopy.width() - length) / 2; x < (imgCopy.width() - length)/ 2 + length; x++){
        for(unsigned int y = (imgCopy.height() - length) / 2; y < (imgCopy.height() - length) / 2 + length; y++){
...
HSLAPixel toqutree::travel(int x, int y, Node * node) {
    ...
    if(node->SE != NULL){           //not the leaf node
...
void toqutree::pruneHelper(Node* cRoot, double tol){
	if(cRoot->SE != NULL){
		double SE_tol = cRoot->SE->avg.dist(cRoot->avg);
		double SW_tol = cRoot->SW->avg.dist(cRoot->avg);
		double NE_tol = cRoot->NE->avg.dist(cRoot->avg);
		double NW_tol = cRoot->NW->avg.dist(cRoot->avg);
		if(SE_tol <= tol && SW_tol <= tol && NE_tol <= tol && NW_tol <= tol){
            clear(cRoot->SE);
			cRoot->SE = NULL;
